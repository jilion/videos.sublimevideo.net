...
describe ContentTypeChecker do
  ...
  describe '#found?' do
    ...
    context 'asset is not found' do
      before { checker.stub(:head).and_return(nil) }
    ...
    context 'asset is found' do
      before { checker.stub(:head).and_return({ 'content-type' => 'video/mp4' }) }
  ...
  describe '#valid_content_type?' do
    ...
    context 'asset has invalid content type' do
      before { checker.stub(:head).and_return({ 'content-type' => 'video/mov' }) }
