...
describe VideoTagTitleFetcher do
  ...
  context "with video title not present and known sources" do
    ...
    before { expect(VideoInfoWrapper).to receive(:new).with(video_id: 'source_id', provider: 'hosting_service') { double(title: 'Title', provider: 'hosting_service') } }
  ...
  context "with video title present and known sources" do
    ...
    before { expect(VideoInfoWrapper).to_not receive(:new) }
  ...
  context "with no title and video sources without title" do
    ...
    before { expect(VideoInfoWrapper).to receive(:new).with(video_id: 'source_id', provider: 'hosting_service') { double(title: nil, provider: 'hosting_service') } }
