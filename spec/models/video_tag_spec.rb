...
describe VideoTag do
  ...
  context "Factory" do
    ...
    its(:sources)         { should have(2).sources }
  ...
  describe "Scopes" do
    ...
    describe ".by_name" do
      let!(:video_tag_a) { create(:video_tag, name: 'a') }
      let!(:video_tag_b) { create(:video_tag, name: 'b') }

      it "sorts by name ASC by default" do
        VideoTag.by_name.first.should eq video_tag_a
      end

      it "sorts by name order given (DESC)" do
        VideoTag.by_name(:desc).first.should eq video_tag_b
      end
    end
  ...
  describe "Validations" do
    ...
    it { should ensure_inclusion_of(:name_origin).in_array(%w[attribute youtube vimeo]).allow_nil }
  ...
  describe "#name=" do
    it "truncates long name" do
      ...
      video_tag.update(name: long_name)
      video_tag.name.size.should eq 255
    end

    it "sets to nil" do
      video_tag.update(name: nil)
