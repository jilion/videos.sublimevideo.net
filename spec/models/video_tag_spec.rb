...
describe VideoTag do
  ...
  describe "Scopes" do
    describe ".last_30_days_active" do
      let!(:old_video_tag) { create(:video_tag, started_at: 31.days.ago) }
      let!(:video_tag) { create(:video_tag, started_at: 30.days.ago) }

      it "returns only recently started video_tags" do
        expect(VideoTag.last_30_days_active).to eq [video_tag]
      end
    end
    describe ".last_90_days_active" do
      let!(:old_video_tag) { create(:video_tag, started_at: 91.days.ago) }
      let!(:video_tag) { create(:video_tag, started_at: 90.days.ago) }
    ...
    describe ".last_365_days_active" do
      let!(:old_video_tag) { create(:video_tag, started_at: 366.days.ago) }
      let!(:video_tag) { create(:video_tag, started_at: 365.days.ago) }
