...
describe VideoTagStartHandlerWorker do
  ...
  it "updates video_tag duration" do
    expect(video_tags).to receive(:update_all).with(started_at: Time.parse(time.to_s), duration: 123456)
    worker.perform(*params)
  end

  it "limits max duration integer" do
    params[2] = { 'vd' => '5461782000', 't' => time.to_s }
    expect(video_tags).to receive(:update_all).with(started_at: Time.parse(time.to_s), duration: nil)
    worker.perform(*params)
  end
end
